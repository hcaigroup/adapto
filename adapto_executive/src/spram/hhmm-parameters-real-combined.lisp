...
(defun create-state-transition-probabilities ()
  (let ((state-transitions (make-hash-table :test 'equalp)))
    ...
    (setf (gethash (string 'DW-table-ST-oven) state-transitions) 0.33)
    (setf (gethash (string 'DW-table-ST-refrigerator) state-transitions) 0.33)
    
    ;; set table
    (setf (gethash (string 'ST-oven-ST-table) state-transitions) 0.99)
    (setf (gethash (string 'ST-table-ST-refrigerator) state-transitions) 0.2475)
    (setf (gethash (string 'ST-refrigerator-ST-table) state-transitions) 0.99)
    (setf (gethash (string 'ST-table-ST-cupboard0) state-transitions) 0.2475)
    (setf (gethash (string 'ST-cupboard0-ST-table) state-transitions) 0.99)
    (setf (gethash (string 'ST-table-ST-drawer) state-transitions) 0.2475)
    (setf (gethash (string 'ST-drawer-ST-table) state-transitions) 0.99)

    ;; transition set-table - clean-table
    (setf (gethash (string 'ST-table-CT-refrigerator) state-transitions) 0.2475)
    (setf (gethash (string 'ST-table-CT-oven) state-transitions) 0.2475)
    
    ;; clean table cornflakes
    (setf (gethash (string 'CT-table-CT-sink) state-transitions) 0.7425)
    (setf (gethash (string 'CT-table-CT-refrigerator) state-transitions) 0.2475)
    (setf (gethash (string 'CT-oven-CT-table) state-transitions) 0.92)
    (setf (gethash (string 'CT-refrigerator-CT-table) state-transitions) 0.99)
    (setf (gethash (string 'CT-sink-CT-table) state-transitions) 0.495)

    ;; transition clean - prepare work
    (setf (gethash (string 'CT-sink-PW-table) state-transitions) 0.495)
    (setf (gethash (string 'CT-oven-PW-table) state-transitions) 0.077)
