...
(defun create-state-transition-probabilities ()
  (let ((state-transitions (make-hash-table :test 'equalp)))
    ...
    ;; drink water plan
    (setf (gethash (string 'DW-bottle-place-DW-table) state-transitions) 0.9)
    (setf (gethash (string 'DW-table-DW-cupboard1) state-transitions) 0.9)
    (setf (gethash (string 'DW-cupboard1-DW-table) state-transitions) 0.9)

    ;; transition: drink - set table
    (setf (gethash (string 'DW-table-STC-oven) state-transitions) 0.9)
    (setf (gethash (string 'DW-table-STQ-refrigerator) state-transitions) 0.9)
    
    ;; set table cornflakes
    (setf (gethash (string 'STC-oven-STC-table) state-transitions) 0.9)
    (setf (gethash (string 'STC-table-STC-refrigerator) state-transitions) 0.3)
    (setf (gethash (string 'STC-refrigerator-STC-table) state-transitions) 0.9)
    (setf (gethash (string 'STC-table-STC-cupboard0) state-transitions) 0.3)
    (setf (gethash (string 'STC-cupboard0-STC-table) state-transitions) 0.9)
    (setf (gethash (string 'STC-table-STC-drawer) state-transitions) 0.3)
    (setf (gethash (string 'STC-drawer-STC-table) state-transitions) 0.9)

    ;; set table quark
    (setf (gethash (string 'STQ-table-STQ-refrigerator) state-transitions) 0.3)
    (setf (gethash (string 'STQ-refrigerator-STQ-table) state-transitions) 0.9)
    (setf (gethash (string 'STQ-table-STQ-cupboard0) state-transitions) 0.3)
    (setf (gethash (string 'STQ-cupboard0-STQ-table) state-transitions) 0.9)
    (setf (gethash (string 'STQ-table-STQ-drawer) state-transitions) 0.3)
    (setf (gethash (string 'STQ-drawer-STQ-table) state-transitions) 0.9)

    ;; transition set-table - clean-table
    (setf (gethash (string 'STQ-table-CTQ-refrigerator) state-transitions) 0.9)
    (setf (gethash (string 'STC-table-CTC-oven) state-transitions) 0.9)
    
    ;; clean table cornflakes
    (setf (gethash (string 'CTC-table-CTC-sink) state-transitions) 0.75)
    (setf (gethash (string 'CTC-table-CTC-refrigerator) state-transitions) 0.25)
    (setf (gethash (string 'CTC-oven-CTC-table) state-transitions) 0.9)
    (setf (gethash (string 'CTC-refrigerator-CTC-table) state-transitions) 0.9)
    (setf (gethash (string 'CTC-sink-CTC-table) state-transitions) 0.9)

    ;; clean table quark
    (setf (gethash (string 'CTQ-table-CTQ-sink) state-transitions) 0.75)
    (setf (gethash (string 'CTQ-table-CTQ-refrigerator) state-transitions) 0.25)
    (setf (gethash (string 'CTQ-refrigerator-CTQ-table) state-transitions) 0.9)
    (setf (gethash (string 'CTQ-sink-CTQ-table) state-transitions) 0.9) 

    ;; transition clean - prepare work
    (setf (gethash (string 'CTQ-sink-PW-table) state-transitions) 0.9)
    (setf (gethash (string 'CTC-sink-PW-table) state-transitions) 0.9)
    
    ;; prepare work
    (setf (gethash (string 'PW-table-PW-table2) state-transitions) 0.9)
    (setf (gethash (string 'PW-table2-PW-door) state-transitions) 0.9)
...
(defun create-state-observation-probabilities ()
  (let ((observation-probabilities (make-hash-table :test 'equalp)))
    ...
    ;; cupboard1
    (setf (gethash (string 'cupboard1-cupboard1) observation-probabilities) 0.077)
    (setf (gethash (string 'cupboard1-sink) observation-probabilities) 0.923)

    ;; cupboard0
    (setf (gethash (string 'cupboard0-cupboard0) observation-probabilities) 0.733)
    (setf (gethash (string 'cupboard0-sink) observation-probabilities) 0.2)
    (setf (gethash (string 'cupboard0-cupboard) observation-probabilities) 0.067)

    ;; sink
    (setf (gethash (string 'sink-sink) observation-probabilities) 0.7)
    (setf (gethash (string 'sink-cupboard1) observation-probabilities) 0.225)
    (setf (gethash (string 'sink-cupboard0) observation-probabilities) 0.025)
    (setf (gethash (string 'sink-drawer) observation-probabilities) 0.025)

    ;; bottle place
    (setf (gethash (string 'bottle-place-bottle-place) observation-probabilities) 0.385)
    (setf (gethash (string 'bottle-place-cupboard1) observation-probabilities) 0.154)
    (setf (gethash (string 'bottle-place-drawer) observation-probabilities) 0.462)

    ;; drawer
    (setf (gethash (string 'drawer-drawer) observation-probabilities) 0.923)
    (setf (gethash (string 'drawer-bottle-place) observation-probabilities) 0.077)

    ;; oven
    (setf (gethash (string 'oven-oven) observation-probabilities) 0.833)
    (setf (gethash (string 'oven-drawer) observation-probabilities) 0.056)
    (setf (gethash (string 'oven-refrigerator) observation-probabilities) 0.111)
    
    ;; fridge
    (setf (gethash (string 'refrigerator-refrigerator) observation-probabilities) 0.821)
    (setf (gethash (string 'refrigerator-oven) observation-probabilities) 0.128)
    (setf (gethash (string 'refrigerator-drawer) observation-probabilities) 0.026)
