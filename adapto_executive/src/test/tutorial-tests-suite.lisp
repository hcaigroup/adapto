...
;; Tests if forward-backward algorithm works correctly using the umbrella example from wikipedia:
;; http://en.wikipedia.org/wiki/Forward%E2%80%93backward_algorithm
(deftest test-fw-bw
    (let ((hmm (make-instance 'hmm)))
      (set-belief hmm (create-umbrella-uniform-state-probabilities))
      (set-start-belief hmm (create-umbrella-uniform-state-probabilities))
...
;; Tests if viterbi algorithm works correctly using the umbrella example from wikipedia
;; http://en.wikipedia.org/wiki/Forward%E2%80%93backward_algorithm
(deftest test-viterbi
    (let ((hmm (make-instance 'hmm)))
      (set-belief hmm (create-umbrella-uniform-state-probabilities))
      (set-start-belief hmm (create-umbrella-uniform-state-probabilities))
      (set-state-transitions hmm (create-umbrella-state-transition-probabilities))
